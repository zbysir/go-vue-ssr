...
func genComponentRenderFunc(c *Compiler, pkgName, name string, file string, srcHash string) []byte {
	...
	f := []byte(fmt.Sprintf("// Code generated by go-vue-ssr: https://github.com/bysir-zl/go-vue-ssr\n// src_hash:%s\n\n"+
		"package %s\n\n"+
...
func GenAllFile(src, desc string, pkg string) (err error) {
	...
	// buildin代码
	code = []byte(fmt.Sprintf("// Code generated by go-vue-ssr: https://github.com/bysir-zl/go-vue-ssr\n\npackage %s\n", pkgName) +
		strings.ReplaceAll(buildInCode, "package xxx", ""))
	err = ioutil.WriteFile(desc+string(os.PathSeparator)+"buildin.go", code, 0666)
	if err != nil {
		return
	}
